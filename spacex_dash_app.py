...
def scatter_chart(site_dropdown,payload_slider):
    if site_dropdown == 'ALL':
        ...
        df = spacex_df[(spacex_df['Launch Site'] == site_dropdown) & (spacex_df['Payload Mass (kg)'] >= payload_slider[0]) & (spacex_df['Payload Mass (kg)'] <= payload_slider[1])]
        fig = px.scatter(df,x='Payload Mass (kg)',y='class',color='Booster Version Category',title='Correlation between Payload and Success for Site '+site_dropdown)
        return fig
